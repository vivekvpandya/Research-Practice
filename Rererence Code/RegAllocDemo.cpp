...
namespace {
  ...
  public:
    ...
    virtual bool runOnMachineFunction(MachineFunction &mf) {
      ...
      const TargetRegisterInfo *tri =  static_cast< const TargetRegisterInfo *>(mf.getTarget().getMCRegisterInfo());

      const TargetInstrInfo *TII = static_cast< const TargetInstrInfo *>(mf.getTarget().getMCInstrInfo());
          ...
          for (unsigned i = 0; i < mi.getNumOperands(); ++i) {
            ...
            if (TargetRegisterInfo::isPhysicalRegister(reg)) {
              ...
              errs() << "physical register " << tri->getName(reg)
                     << " with class " << tri->getRegClassName(trc) << "\n";
            }
            else {
              ...
              if (mo.isUse()) {
                ...
                TII->loadRegFromStackSlot(mbb, mi, useReg, frameIndex, trc, tri);
                mo.setReg(useReg);
                useReg = 38;  // ECX
              }
              else {
                ...
                TII->storeRegToStackSlot(mbb, nextI, defReg, true, frameIndex, trc, tri);
